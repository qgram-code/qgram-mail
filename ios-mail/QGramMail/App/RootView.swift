...
struct RootView: View {
    ...
    var body: some View {
        ...
        .animation(.easeOut(duration: 0.22), value: session.stage)
        .task {
            store.session = session
            store.previewsEnabled = settings.showPreviews
            await session.restore()
        }
        .onChange(of: settings.showPreviews) { value in
            store.previewsEnabled = value
        }
        .onChange(of: session.stage) { stage in
            switch stage {
            case .ready: Task { await store.loadIfNeeded() }
            case .login: store.reset()
            default: break
            }
        }
...
/// Экран запуска, пока проверяется сохранённый токен.
@MainActor
    ...
    var body: some View {
        VStack(spacing: 18) {
            Image(systemName: "envelope")
                ...
                .foregroundStyle(settings.accent.base)
            ProgressView()
